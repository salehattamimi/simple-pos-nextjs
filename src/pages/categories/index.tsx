...
const CategoriesPage: NextPageWithLayout = () => {
  ...
  const { mutate: createCategory } = api.category.createCategory.useMutation({
    onSuccess: async () => {
      ...
      alert("Successfilly created a new Category");
  ...
  const {mutate: deleteCategoryById} = api.category.deleteCategoryById.useMutation({
    onSuccess: async ()=>{
      ...
      alert("Successfilly deleted Category");
      setCategoryToDelete(null);
    }
  })

  const {mutate: editCategoryById} = api.category.editCategory.useMutation({
     onSuccess: async () => {
      await apiUtils.category.getCategories.invalidate();

      alert("Successfilly edit a Category");
        ...
        {categories?.map((category) => {
          return (
            <CategoryCatalogCard
              ...
              onEdit = {()=> handleClickEditCategory ({id: category.id, name:category.name})}
            />
          );
        })}
      </div>
